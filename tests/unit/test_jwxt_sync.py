...
class TestRefreshJwxtState:
    ...
    def test_safe_url_parts_excludes_query_and_fragment(self):
        domain, path = _safe_url_parts(
            "https://jw.example.edu/index.html?private=value#fragment"
        )

        assert domain == "jw.example.edu"
        assert path == "/index.html"
        assert "private" not in f"{domain}{path}"

    def test_atomic_cookie_write_does_not_return_cookie_values(self, tmp_path):
